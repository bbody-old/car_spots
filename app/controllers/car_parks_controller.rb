class CarParksController < ApplicationController
  ...
  def show
    @booking = Booking.new
    if @car_park.nil?
      @booking.car_park = @car_park.id
    else
      @booking.car_park = @car_park
    end
  end
  ...
  private
    ...
    def car_park_params
      params.require(:car_park).permit(:name, :longitude, :lattitude, :address, :user_id, :description, :electrical, :large, :covered)
    end
end
